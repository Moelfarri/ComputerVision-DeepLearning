...
def pre_process_images(X: np.ndarray):
    ...
    """
    #[0,255] to [-1,1] conversion and reshaping:
...
def cross_entropy_loss(targets: np.ndarray, outputs: np.ndarray) -> float:
    ...
    average_cost = -(1/samples)*(np.dot(targets.T,np.log(outputs)) + np.dot((1-targets).T,np.log(1-outputs)))
    
    #remove single array formation
    average_cost = np.squeeze(average_cost)
...
class BinaryModel:
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        #activation w*X and activation function - sigmoid such that we get y_hat:
        output = np.array([1/(1+np.exp(-X[i].dot(self.w))) for i in range(len(X))])
        return output
    ...
    def backward(self, X: np.ndarray, outputs: np.ndarray, targets: np.ndarray) -> None:
        ...
        Args:
            X: images of shape [batch size, 785]
            outputs: outputs of model of shape: [batch size, 1]  ----- y_hat 
            targets: labels/targets of each image of shape: [batch size, 1] ---- y
        """
        
        #normalization factor and gradient updated
        samples   = outputs.shape[0]
        self.grad =  -(1/samples)*np.dot(X.T,(targets-outputs))
