...
class SoftmaxModel:
    ...
    def __init__(self, l2_reg_lambda: float):
        ...
        self.I = 785 #Specific to image pixels of this dataset

        # Define number of output nodes
        self.num_outputs = 10
        self.w = np.zeros((self.I, self.num_outputs))
        self.grad = None #Gradient size (785,10) -> (batchsize,num_outputs)
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        out = (np.exp(X.dot(self.w)) / np.array([np.sum(np.exp(X.dot(self.w)),  axis=1)]).T)
        return out
    ...
    def backward(self, X: np.ndarray, outputs: np.ndarray, targets: np.ndarray) -> None:
        ...
        #loss gradient with regulization gradient term
        self.grad = -(1/samples)*np.dot(X.T,(targets-outputs)) + 2*self.l2_reg_lambda*self.w
