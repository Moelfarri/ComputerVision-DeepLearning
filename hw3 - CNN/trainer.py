...
def compute_loss_and_accuracy(
    ...
    with torch.no_grad():
        for (X_batch, Y_batch) in dataloader:
            ...
            #compute accuracy
            _, preds = output_probs.max(1) #returns tuple (max_value, max_indice)
            correct_preds += (Y_batch == preds).sum().item() #item turns 1x1 tensor to scalar
            acc_samples   += preds.size(0) 
                
            # Compute Loss 
            loss     += loss_criterion(output_probs, Y_batch).item() #item turns 1x1 tensor to scalar
            loss_samples += 1

    accuracy     = correct_preds/acc_samples
    average_loss = loss/loss_samples
...
class Trainer:
    ...
    def __init__(self,
        ...
        if self.scheduling:
            #step size = how many epochs before the learning rate decreases by learningrate*gamma
    ...
    def validation_step(self):
        ...
        #training accuracy
        _, training_acc = compute_loss_and_accuracy(
        ...
        #register last data
        self.trainloss = list(list(self.train_history["loss"].items())[-1])[-1]
        self.valloss = list(list(self.validation_history["loss"].items())[-1])[-1]
        self.testloss = list(list(self.test_history["loss"].items())[-1])[-1]
        self.trainacc = list(list(self.train_history["accuracy"].items())[-1])[-1]
        self.valacc = list(list(self.validation_history["accuracy"].items())[-1])[-1]
        self.testacc = list(list(self.test_history["accuracy"].items())[-1])[-1]
